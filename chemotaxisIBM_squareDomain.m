...
for iteration = 2:maxIteration
    ...
    if ~rem(iteration, Options.regressionTimeStep)
        ...
        try
            ...
            beta0 = regress(...
                ...
                [ones(size(midXbins(bacterialDistribution>0),2),1),...
    ...
    if checkSteadyState
        ...
        if ~isempty(ind) % && ~exist('maxTime','var')
            ...
            % Estimate steady state for each of the two parameters of the
            % exponential distribution:
            for bi = 1:2
                [betaSS(bi), timeSS(bi)] = steadystate(...
                    beta(filter,bi),...
                    regressionTimes(filter),...
                    splineOptions(bi), Options.steady_state_tol);
            end
...
else % Estimate steady states at the end of the simulation when simulation
     % time is imposed
...
%% Compute population level chemotactic population parameters with uncertainties
    ...
    function [betaSS, timeSS] = steadystate(beta, times, splineOptions, tolerance)
        %Estimates steady state from the time series of beta. First, it adjusts
        %a spline to smooth the curve. From that spline it computes the point at
        %which the relative change is less than tolerance per hour.
        splineOptions.Knots = linspace(...
            log10(times(1)),...
            log10(times(end)),...
            max([5 floor(numel(times)/20)]));
        splineFit = slmengine(log10(times),...
            log10(beta), splineOptions); % actually does the spline fit

        try
            splineFitTime =...
                10^splineFit.knots(1):1:10^splineFit.knots(end);
            splineFitbeta =...
                10.^slmeval(log10(splineFitTime), splineFit);
            relativeChange =...
                abs(gradient(splineFitbeta,splineFitTime))./splineFitbeta.*3600; % relative change in hours^-1
            timeSS = splineFitTime(...
                find(relativeChange>tolerance,1,'last'));
            betaSS = 10.^slmeval(log10(timeSS), splineFit);
        catch
            timeSS = nan;
            betaSS = nan;
        end
